...
| `history.training_sessions` | `trainning/` (re-parse) | 近 7 天  | 一次性读取近 28 天（≈10 次）的完整训练记录，下游 Agent 各取所需窗口 |
...
**原则**：Performance Agent 先算指标再调 LLM 生成 summary。Coach Agent 可基于 summary 或原始指标做决策。
...
这些问题属于 Performance Limitation Analysis 的职责范围（V2）。
...
|------|------|------|
| `name` | `str` | 状态标识符 |
| `priority` | `int` | 响应优先级，1（最高）~ 3（最低），Coach Agent 据此排序 |
